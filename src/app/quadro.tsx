...
export default function TelaQuadro() {
  ...
  // Função para formatar o texto do WhatsApp e disparar o compartilhamento nativo
  const handleCompartilharRelatorio = async () => {
    ...
    circuitos.forEach((c) => {
      const unidade =
        c.tipo === "tue" && c.potenciaWatts
          ? `${c.potenciaWatts}W`
          : `${c.potenciaVA} VA`;
      textoRelatorio += `• ${c.nome}: ${unidade}\n`;
    });
  ...
  const obtenerCorTag = (tipo: "iluminacao" | "tug" | "tue") => {
      ...
      >
        {resultadoQDC ? (
          <View style={styles.quadroContainer}>
            {/* --- SEÇÃO: LISTA DE CIRCUITOS ACUMULADOS --- */}
                ...
                <View key={circuito.id} style={styles.itemCircuito}>
                  <View style={styles.itemEsquerda}>
                    <View
                      style={[
                        styles.tagTipo,
                        { backgroundColor: obtenerCorTag(circuito.tipo) },
                      ]}
                    >
            ...
            {/* --- SEÇÃO: RESULTADO DO ALIMENTADOR GERAL --- */}
            <Text style={styles.subtitulo}>
